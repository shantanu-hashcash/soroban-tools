...
fn sc_address_to_json(v: &ScAddress) -> Value {
    match v {
        ScAddress::Account(AccountId(PublicKey::PublicKeyTypeEd25519(Uint256(k)))) => {
            Value::String(hcnet_strkey::ed25519::PublicKey(*k).to_string())
        }
        ScAddress::Contract(Hash(h)) => Value::String(hcnet_strkey::Contract(*h).to_string()),
    }
}

fn sc_address_from_json(s: &str) -> Result<ScVal, Error> {
    hcnet_strkey::Strkey::from_string(s)
        .map_err(|_| Error::InvalidValue(Some(ScType::Address)))
        .map(|parsed| match parsed {
            hcnet_strkey::Strkey::PublicKeyEd25519(p) => Some(ScVal::Address(
                ScAddress::Account(AccountId(PublicKey::PublicKeyTypeEd25519(Uint256(p.0)))),
            )),
            hcnet_strkey::Strkey::Contract(c) => {
...
mod tests {
    ...
    use hcnet_xdr::curr::ScSpecTypeBytesN;
