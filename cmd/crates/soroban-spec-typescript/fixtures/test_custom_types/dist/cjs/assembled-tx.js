...
const hcnet_sdk_1 = require("hcnet-sdk");
Object.defineProperty(exports, "Address", { enumerable: true, get: function () { return hcnet_sdk_1.Address; } });
...
class AssembledTransaction {
    ...
    static fromJSON(options, { tx, simulationResult, simulationTransactionData }) {
        ...
        txn.raw = hcnet_sdk_1.TransactionBuilder.fromXDR(tx, options.networkPassphrase);
        txn.simulationResult = {
            auth: simulationResult.auth.map(a => hcnet_sdk_1.xdr.SorobanAuthorizationEntry.fromXDR(a, 'base64')),
            retval: hcnet_sdk_1.xdr.ScVal.fromXDR(simulationResult.retval, 'base64'),
        };
        txn.simulationTransactionData = hcnet_sdk_1.xdr.SorobanTransactionData.fromXDR(simulationTransactionData, 'base64');
        return txn;
    }
    constructor(options) {
        this.options = options;
        this.server = new hcnet_sdk_1.SorobanRpc.Server(this.options.rpcUrl, {
            allowHttp: this.options.rpcUrl.startsWith("http://"),
        });
    }
    static async fromSimulation(options) {
        const tx = new AssembledTransaction(options);
        const contract = new hcnet_sdk_1.Contract(options.contractId);
        tx.raw = new hcnet_sdk_1.TransactionBuilder(await tx.getAccount(), {
            fee: options.fee?.toString(10) ?? hcnet_sdk_1.BASE_FEE,
            networkPassphrase: options.networkPassphrase,
        })
            .addOperation(contract.call(options.method, ...(options.args ?? [])))
            .setTimeout(hcnet_sdk_1.TimeoutInfinite)
            .build();
    ...
    simulate = async () => {
        ...
        if (hcnet_sdk_1.SorobanRpc.Api.isSimulationSuccess(this.simulation)) {
            this.raw = hcnet_sdk_1.SorobanRpc.assembleTransaction(this.raw, this.simulation).build();
        }
        return this;
    };
    get simulationData() {
        ...
        if (hcnet_sdk_1.SorobanRpc.Api.isSimulationError(simulation)) {
            throw new Error(`Transaction simulation failed: "${simulation.error}"`);
        }
        if (hcnet_sdk_1.SorobanRpc.Api.isSimulationRestore(simulation)) {
    ...
    getWallet = async () => {
        return this.options.wallet ?? (await Promise.resolve().then(() => require("@hcnet/freighter-api"))).default;
    };
    getPublicKey = async () => {
    ...
    getAccount = async () => {
        ...
        return publicKey
            ...
            : new hcnet_sdk_1.Account(exports.NULL_ACCOUNT, "0");
    };
    /**
    ...
    getStorageExpiration = async () => {
        const entryRes = await this.server.getLedgerEntries(new hcnet_sdk_1.Contract(this.options.contractId).getFootprint());
        if (!entryRes.entries ||
    ...
    needsNonInvokerSigningBy = async ({ includeAlreadySigned = false, } = {}) => {
        ...
        return [...new Set((rawInvokeHostFunctionOp.auth ?? []).filter(entry => entry.credentials().switch() ===
                hcnet_sdk_1.xdr.SorobanCredentialsType.sorobanCredentialsAddress() &&
                (includeAlreadySigned ||
                    entry.credentials().address().signature().switch().name === 'scvVoid')).map(entry => hcnet_sdk_1.StrKey.encodeEd25519PublicKey(entry.credentials().address().address().accountId().ed25519())))];
    };
    preImageFor(entry, signatureExpirationLedger) {
        const addrAuth = entry.credentials().address();
        return hcnet_sdk_1.xdr.HashIdPreimage.envelopeTypeSorobanAuthorization(new hcnet_sdk_1.xdr.HashIdPreimageSorobanAuthorization({
            networkId: (0, hcnet_sdk_1.hash)(buffer_1.Buffer.from(this.options.networkPassphrase)),
    ...
    expiration = this.getStorageExpiration()) => {
        ...
        for (const [i, entry] of authEntries.entries()) {
            ...
            const pk = hcnet_sdk_1.StrKey.encodeEd25519PublicKey(entry.credentials().address().address().accountId().ed25519());
            // this auth entry needs to be signed by a different account
            // (or maybe already was!)
            if (pk !== publicKey)
                continue;
            authEntries[i] = await (0, hcnet_sdk_1.authorizeEntry)(entry, async (preimage) => buffer_1.Buffer.from(await wallet.signAuthEntry(preimage.toXDR('base64')), 'base64'), await expiration, this.options.networkPassphrase);
        }
    };
    get isReadCall() {
    ...
    send = async (secondsToWait = 10) => {
        ...
        this.sendTransactionResponseAll = await withExponentialBackoff(async (previousFailure) => {
            if (previousFailure) {
                ...
                this.assembled.raw = new hcnet_sdk_1.TransactionBuilder(await this.assembled.getAccount(), {
                    fee: this.assembled.raw.fee,
                    networkPassphrase: this.options.networkPassphrase,
                })
                    .setTimeout(hcnet_sdk_1.TimeoutInfinite)
                    .addOperation(hcnet_sdk_1.Operation.invokeHostFunction({ ...op, auth: op.auth ?? [] }))
                    .build();
            ...
            this.signed = hcnet_sdk_1.TransactionBuilder.fromXDR(signature, this.options.networkPassphrase);
        ...
        this.getTransactionResponseAll = await withExponentialBackoff(() => this.server.getTransaction(hash), resp => resp.status === hcnet_sdk_1.SorobanRpc.Api.GetTransactionStatus.NOT_FOUND, secondsToWait);
        this.getTransactionResponse = this.getTransactionResponseAll[this.getTransactionResponseAll.length - 1];
        if (this.getTransactionResponse.status === hcnet_sdk_1.SorobanRpc.Api.GetTransactionStatus.NOT_FOUND) {
