...
func TestLoadConfigPathPrecedence(t *testing.T) {
	...
	require.NoError(t, cmd.ParseFlags([]string{
		...
		"--hcnet-core-binary-path", "/usr/overridden/hcnet-core",
		"--network-passphrase", "CLI test passphrase",
	}))

	require.NoError(t, cfg.SetValues(func(key string) (string, bool) {
		switch key {
		case "HCNET_CORE_BINARY_PATH":
			return "/env/hcnet-core", true
		case "DB_PATH":
	...
	assert.Equal(t, "/opt/hcnet/soroban-rpc/etc/hcnet-captive-core.cfg", cfg.CaptiveCoreConfigPath, "should read values from the config path file")
	assert.Equal(t, "CLI test passphrase", cfg.NetworkPassphrase, "cli flags should override --config-path values")
	assert.Equal(t, "/usr/overridden/hcnet-core", cfg.HcnetCoreBinaryPath, "cli flags should override --config-path values and env vars")
