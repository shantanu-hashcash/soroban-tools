...
const basicToml = `
HISTORY_ARCHIVE_URLS = [ "http://history-futurenet.hcnet.org" ]
NETWORK_PASSPHRASE = "Test SDF Future Network ; October 2022"

# testing comments work ok
HCNET_CORE_BINARY_PATH = "/usr/bin/hcnet-core"
CAPTIVE_CORE_STORAGE_PATH = "/etc/hcnet/soroban-rpc"
CAPTIVE_CORE_CONFIG_PATH = "/etc/hcnet/soroban-rpc/captive-core.cfg"
`
...
func TestBasicTomlReading(t *testing.T) {
	...
	assert.Equal(t, []string{"http://history-futurenet.hcnet.org"}, cfg.HistoryArchiveURLs)
	assert.Equal(t, network.FutureNetworkPassphrase, cfg.NetworkPassphrase)
	assert.Equal(t, "/etc/hcnet/soroban-rpc", cfg.CaptiveCoreStoragePath)
	assert.Equal(t, "/etc/hcnet/soroban-rpc/captive-core.cfg", cfg.CaptiveCoreConfigPath)
}
...
func TestBasicTomlWriting(t *testing.T) {
	...
	assert.Contains(t, out, "HCNET_CORE_TIMEOUT = \"2s\"")
	assert.Contains(t, out, "HCNET_CAPTIVE_CORE_HTTP_PORT = 11626")
	...
	assert.Contains(t, out, "# Network passphrase of the Hcnet network transactions should be signed for")
