...
use hcnet_strkey::ed25519::PrivateKey;
...
///
/// Might return an error
pub fn contract_id_from_str(contract_id: &str) -> Result<[u8; 32], hcnet_strkey::DecodeError> {
    hcnet_strkey::Contract::from_string(contract_id)
        .map(|strkey| strkey.0)
        .or_else(|_| {
            ...
            soroban_spec_tools::utils::padded_hex_from_str(contract_id, 32)
                .map_err(|_| hcnet_strkey::DecodeError::Invalid)?
                .try_into()
                .map_err(|_| hcnet_strkey::DecodeError::Invalid)
        })
        .map_err(|_| hcnet_strkey::DecodeError::Invalid)
...
pub(crate) fn parse_secret_key(
    s: &str,
) -> Result<ed25519_dalek::SigningKey, hcnet_strkey::DecodeError> {
...
pub mod parsing {
    ...
    pub fn parse_account_id(str: &str) -> Result<AccountId, Error> {
        let pk_bytes = hcnet_strkey::ed25519::PublicKey::from_string(str)
...
mod tests {
    ...
    fn test_contract_id_from_str() {
        ...
        match contract_id_from_str("foobar") {
            ...
            Err(err) => assert_eq!(err, hcnet_strkey::DecodeError::Invalid),
        }
