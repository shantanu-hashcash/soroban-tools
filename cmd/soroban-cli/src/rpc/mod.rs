...
pub enum Error {
    #[error(transparent)]
    InvalidAddress(#[from] hcnet_strkey::DecodeError),
    ...
    pub async fn get_account(&self, address: &str) -> Result<AccountEntry, Error> {
        ...
        let key = LedgerKey::Account(LedgerKeyAccount {
            account_id: AccountId(PublicKey::PublicKeyTypeEd25519(Uint256(
                hcnet_strkey::ed25519::PublicKey::from_string(address)?.0,
            ))),
        });
...
soroban config identity fund {address} --helper-url <url>"#
    ...
    pub async fn get_contract_data(
        ...
        if entries.is_empty() {
            let contract_address = hcnet_strkey::Contract(*contract_id).to_string();
