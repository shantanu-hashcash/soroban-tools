...
const PUBLIC_NETWORK_PASSPHRASE: &str = "Public Global Hcnet Network ; September 2015";
...
pub enum Error {
    ...
    #[error("the deployed smart contract {wasm} was built with Soroban Rust SDK v{version}, a release candidate version not intended for use with the Hcnet Public Network. To deploy anyway, use --ignore-checks")]
    ...
    async fn run_against_rpc_server(&self, contract: &[u8]) -> Result<Hash, Error> {
        ...
        if let Some(rs_sdk_ver) = get_contract_meta_sdk_version(wasm_spec) {
                ...
                && network.network_passphrase == PUBLIC_NETWORK_PASSPHRASE
            {
                ...
                && network.network_passphrase == PUBLIC_NETWORK_PASSPHRASE
            {
                tracing::warn!("the deployed smart contract {path} was built with Soroban Rust SDK v{rs_sdk_ver}, a release candidate version not intended for use with the Hcnet Public Network", path = self.wasm.wasm.display());
            }
        }
